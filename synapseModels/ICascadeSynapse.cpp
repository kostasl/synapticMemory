...
//If Metaplastic Allocation Criteria -- Then Compare Metaplastic Cycles against threshold
...
\\NOTE: REMEMBER TO CALL THIS FUNCTION BEFORE strength is changed in order to evaluate the strength over which the cycles occured
*/
void ICascadeSynapse::saveMetaplasticDistribution()
{
	...
	//assert(penumTrackedStrength == ICascadeSynapse::SYN_STRENGTH_STRONG); //testing
...
void ICascadeSynapse::reset()
{
	...
	if (r < 0.5)
		...
		//penumTrackedStrength = penumStartStrength = penumStrength = SYN_STRENGTH_STRONG;
	else
		//penumTrackedStrength =  penumStartStrength = penumStrength = SYN_STRENGTH_WEAK;
