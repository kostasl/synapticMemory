...
for (it = vpSyns.begin();it!=vpSyns.end();++it)
{
	...
	if (bresetAllocationThreshold) //Just Before 1st Encoding of tracked Pattern
	{
		...
		oCSyn->disableMetaplasticCounting(); //Do Not Save this Distribution - Just Reset
		oCSyn->resetMetaplasticCounter();
...
*/
	...
	t_simRet Ret;
	Ret.first.first 	= dLastSignalValue;
	Ret.first.second 	= dLastVarValue;
	Ret.second.first	= dLastPKAValue;
	Ret.second.second	= dLastPKAVar;
