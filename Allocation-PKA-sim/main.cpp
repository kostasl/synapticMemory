...
void runAllocSignalVsRepetition(int modelType,double ts, long trials, int trackedMemIndex, int RepMemoryIndex, int iRepMemoryCount, int FilterSize, long synapsesPopulation, long lSimtimeSeconds, double dEncodingRate, string inputFile)
{
	...
	//Write Header
	ofile << "#RepTime\tAllocSNR\tAllocSignal\tAllocVariance\tAllocThreshold\tPKALevel\tPKAVariance" << endl;
	...
	while (dRepIntervalsecs <= MaxRepTime)
	{
		...
		ofile << iabsRepTime << "\t"<< AllocSignal.first.first/sqrt(AllocSignal.first.second) << "\t" << AllocSignal.first.first << "\t" << AllocSignal.first.second << "\t" << g_fAllocHThres << "\t" << AllocSignal.second.first << "\t" << AllocSignal.second.second << endl;
		//Clear Object Memory
		cout << "R.I:" << iabsRepTime << " PKA:" << AllocSignal.second.first << endl;

		return_temporary_buffer(mem_buffer);

		clock_t finish2 = std::clock();
