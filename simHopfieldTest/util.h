...
template <class T>
void makeLogFileNames(vector<string>& vLogFiles,unsigned int ciInitPeriod,int iCascadeSize,int RepCount,double mdRepInterval,double dSignalThres,uint trials, int iSynCount)
{
	if (vLogFiles.empty())
	...
	oss << "_NS" << iSynCount << "_I%d" << "_n" <<  iCascadeSize << "_r" << RepCount << "x" << mdRepInterval;
	...
	MakeListOfFiles(vLogFiles,ciInitPeriod,iCascadeSize,mdRepInterval,0.5,iSynCount);
...
{
	makeLogFileNames<T>(vLogFiles,ciInitPeriod,iCascadeSize,mdRepInterval,dSignalThres,trials, iSynCount);

	char buffFFile[400];
	sprintf(buffFFile,vLogFiles[10].c_str(),FilterState); //The Value passed Should be the Starting Filter State - Not The initPeriod
	string sDistrib(buffFFile);
	vLogFiles[10] = sDistrib;
}
