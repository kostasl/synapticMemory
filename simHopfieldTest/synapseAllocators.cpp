...
template <>
ICascadeSynapse* allocSynapseArray<synapseCascade>(char* buffer,int iSynCount,int iCascadeSize, gsl_rng* prng_r, float StimRate)
{
...
//   cout << "Bytes Required:" << sizeof(synapseCascade)*iSynCount << endl;
	...
	for (int i =0;i<iSynCount;i++)
	{
		...
		if (bFixedStartState)
		{
			...
			//CascadeSize
			new(pseg) synapseCascade(iCascadeSize,1, prng_r);
		}
		else//Starting Strength is Random
			new(pseg) synapseCascade(iCascadeSize,prng_r);
	}
